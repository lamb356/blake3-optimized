...
async function runBenchmarks() {
  // Initialize SIMD
  console.log('Initializing SIMD...');
  const simdEnabled = await blake3.initSimd();
  console.log('SIMD enabled:', simdEnabled);

  // Warmup
  console.log('Warming up...\n');
  for (let i = 0; i < 1000; i++) {
    blake3.hash(new Uint8Array(4096));
  }
  ...
  console.log('- SIMD should provide ~1.5-2x speedup on supported platforms');
}
